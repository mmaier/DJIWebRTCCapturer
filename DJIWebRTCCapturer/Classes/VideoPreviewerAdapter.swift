...
protocol VideoPreviewerAdapter {
    var delegate: VideoFrameProcessor? { get set }
    func start()
    func stop()
    func setupFrameControlHandler()
}

class DroneVideoPreviewerAdapter: NSObject, VideoPreviewerAdapter {
    ...
    private var isAircraft = false
    private var cameraMode: DJICameraMode = .unknown
    private var photoRatio: DJICameraPhotoAspectRatio = .ratioUnknown
    private var isLightbridge2 = false
    private var productName: String?
    private var cameraName: String?
    private var calibrateLogic: DecodeImageCalibrateLogic
    ...
    override init() {
        self.calibrateLogic = DroneDecodeImageCalibrateLogic()
        super.init()
    ...
    @objc fileprivate func updateInfo() {
        ...
        guard let cameraModel = camera else { return }
    ...
    fileprivate func updateEncodeType() {
        ...
        if isUsingLightbridge2(for: productName, aircraft: isAircraft, camera: cameraName) {
            previewer?.encoderType = ._LightBridge2
            return
        }
        
        let encodeType = getDataSource(with: cameraName, aircraft: isAircraft)
...
extension DroneVideoPreviewerAdapter: DJIVideoFeedListener {
...
// MARK: lightbridge2
extension DroneVideoPreviewerAdapter {
...
// MARK: delegate
extension DroneVideoPreviewerAdapter: DJIVideoFeedSourceListener {
...
extension DroneVideoPreviewerAdapter: DJIVideoPreviewerFrameControlDelegate {
    ...
    func isNeedFitFrameWidth() -> Bool {
        let displayName = camera?.displayName
...
extension DroneVideoPreviewerAdapter {
    ...
    fileprivate func isUsingLightbridge2(for product: String?, aircraft: Bool, camera: String?) -> Bool {
    ...
    fileprivate func getDataSource(with camera: String?, aircraft: Bool) -> H264EncoderType {
        switch camera {
        case DJICameraDisplayNameX3:
            if let cameraModel = self.camera {
                /**
        ...
        case DJICameraDisplayNameZ3:
            return ._A9_OSMO_NO_368
        case DJICameraDisplayNameX5,
             DJICameraDisplayNameX5R:
            return ._DM368_inspire
        case DJICameraDisplayNamePhantom3ProfessionalCamera:
            return ._DM365_phamtom3x
        case DJICameraDisplayNamePhantom3AdvancedCamera:
            return ._A9_phantom3s
        case DJICameraDisplayNamePhantom3StandardCamera:
            return ._A9_phantom3c
        case DJICameraDisplayNamePhantom4Camera:
            return ._1860_phantom4x
        case DJICameraDisplayNameMavicProCamera:
        ...
        case DJICameraDisplayNameSparkCamera:
            return ._1860_phantom4x
        case DJICameraDisplayNameZ30:
            return ._GD600
        case DJICameraDisplayNamePhantom4ProCamera,
             DJICameraDisplayNamePhantom4AdvancedCamera,
             DJICameraDisplayNameX5S,
             DJICameraDisplayNameX4S,
             DJICameraDisplayNameX7,
             DJICameraDisplayNamePayload:
            return ._H1_Inspire2
        case DJICameraDisplayNameMavicAirCamera:
            return ._MavicAir
        default:
            return ._unknown
        }
